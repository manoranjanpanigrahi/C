...
int main()
{
   ...
   printf("\n Input the number of nodes for Doubly Linked List A : ");
   scanf("%d", &n);
   DlListcreation(n); 
   a=1;
   displayDlList(a);
   printf("\n Input the position ( 1 to %d ) to insert a new node : ",n+1);
   scanf("%d", &insPlc);
   printf("\n Input data for the position %d : ", insPlc);
   scanf("%d", &num1);
...
void displayDlList(int m)
{
   ...
   else
   {
      ...
      if (m==1)
      {
      printf("\n Data entered in the Doubly Linked list A are :\n");
      }
      else
      {
         printf("\n After insertion the new Doubly List list A is :\n");   
      }
      while(tmp != NULL)
      {
         printf(" node %d : %d\n", n, tmp->num);
